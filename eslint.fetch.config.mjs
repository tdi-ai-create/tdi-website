 ...
 *
 * This config exists so `npm run check:fetch` can be a gate today, without
 * waiting for the rest of the lint debt to be paid down.
 *
 * Run:  npm run check:fetch
 */

import tsParser from '@typescript-eslint/parser';
import tsPlugin from '@typescript-eslint/eslint-plugin';
import reactHooksPlugin from 'eslint-plugin-react-hooks';
import noUncheckedFetch from './eslint-rules/no-unchecked-fetch.mjs';
...
export default [
  ...
  },
  {
    ...
    plugins: {
      tdi: { rules: { 'no-unchecked-fetch': noUncheckedFetch } },
