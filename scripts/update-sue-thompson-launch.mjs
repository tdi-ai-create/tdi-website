/**
 * Script to update Sue Thompson's creator record for course launch
 * Run with: node scripts/update-sue-thompson-launch.mjs
 */
...
async function main() {
  ...
  console.log('Updating Sue Thompson for Course Launch');
  console.log('='.repeat(60));

  // 1. Find Sue Thompson by email
  console.log('\n1. Finding Sue Thompson...');
  ...
  if (findError || !creator) {
    console.error('Error finding Sue Thompson:', findError?.message || 'Not found');
    process.exit(1);
  }

  console.log(`✓ Found creator: ${creator.name} (ID: ${creator.id})`);
  console.log(`  Current discount_code: ${creator.discount_code || '(not set)'}`);
  console.log(`  Current course_url: ${creator.course_url || '(not set)'}`);
  console.log(`  Current phase: ${creator.current_phase}`);

  // 2. Update course_url and verify/update discount_code
  ...
  // Check if discount_code needs updating
  if (creator.discount_code !== 'THOMPSON') {
    updates.discount_code = 'THOMPSON';
    console.log(`  Updating discount_code from "${creator.discount_code}" to "THOMPSON"`);
  } else {
    console.log(`  discount_code is already "THOMPSON" ✓`);
  }
  ...
  console.log('✓ Updated course_url to:', updates.course_url);
  ...
  for (const cm of creatorMilestones) {
    ...
    if (completeError) {
      ...
      console.log(`  ✓ Completed: ${milestone?.name || milestone?.title || milestoneId}`);
      completedCount++;
    }
  }
  ...
  for (const cm of creatorMilestones) {
    ...
    if (optionalError) {
      ...
      const milestoneName = cm.milestone?.name || cm.milestone?.title || cm.milestone_id;
  ...
  console.log(`  Course URL: ${updatedCreator.course_url}`);
  console.log(`  Discount Code: ${updatedCreator.discount_code}`);
  ...
  console.log('\n' + '='.repeat(60));
  console.log('Sue Thompson launch update complete!');
  console.log('='.repeat(60));
}
