...
// Same fake personas as quick win seed
const PERSONAS = [
...
// Same body text pools as quick win seed
const TRIED_IT = [
...
async function main() {
  ...
  const { count: existingCount } = await supabase
    .from('course_responses')
    .select('id', { count: 'exact', head: true });

  console.log(`Existing course_responses: ${existingCount}`);

  if ((existingCount || 0) > 10) {
    console.log('Already seeded. Delete existing seed rows first if you want to re-seed.');
    console.log("Run: DELETE FROM course_responses WHERE user_id LIKE 'a%';");
    return;
  }
  ...
  for (const course of courses || []) {
    ...
    if (insertError) {
      console.error(`Error seeding ${course.slug}:`, insertError.message);
    } else {
  ...
  console.log(`\nDone! Inserted ${totalInserted} responses across ${courses?.length} courses.`);

  // Verify
  const { count: finalCount } = await supabase
    .from('course_responses')
    .select('id', { count: 'exact', head: true });
  console.log(`Total course_responses in table: ${finalCount}`);
}
