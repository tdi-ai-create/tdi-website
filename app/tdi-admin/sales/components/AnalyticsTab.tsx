...
function PulseSection({ pulse }: { pulse: AnalyticsData['pulse'] }) {
  const cards = [
    { label: 'Pipeline', value: `$${(pulse.totalPipeline / 1000).toFixed(0)}K`, color: '#0a0f1e' },
    { label: 'Factored', value: `$${(pulse.factored / 1000).toFixed(0)}K`, color: '#10B981' },
    { label: 'Active deals', value: String(pulse.activeCount), color: '#0a0f1e' },
    { label: 'Avg deal size', value: `$${(pulse.avgDealSize / 1000).toFixed(0)}K`, color: '#0a0f1e' },
  ...
  return (
    <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: 16, padding: 24 }}>
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: '#0a0f1e' }}>{title}</div>
