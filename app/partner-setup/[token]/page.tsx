...
function PartnerSetupContent() {
  ...
  // Accept invite and redirect to intake
  const acceptInviteAndRedirect = async (userId: string, userEmail: string) => {
    try {
  ...
  const handleGoogleSignIn = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/partner-setup/${token}`,
        queryParams: {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden">
        ...
        <div className="p-8">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-3.5 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 hover:shadow-sm transition-all duration-200 text-gray-700 font-medium disabled:opacity-50 mb-4"
          >
            {isGoogleLoading ? (
