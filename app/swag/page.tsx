...
import { SWAG_PRODUCTS, COLLECTIONS, getProductsByCollection, getStickers, type SwagProduct } from '@/lib/swag/products';
...
function ImageCarousel({ images, size = 'card' }: { images: string[]; size?: 'card' | 'modal' }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{
        width: '100%', aspectRatio: '1', borderRadius: isModal ? 12 : 8, background: C.gray,
        ...
        {images.length > 1 && (
          <>
            <button onClick={(e) => { e.stopPropagation(); setIdx(i => i > 0 ? i - 1 : images.length - 1); }}
              style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.85)', border: 'none', cursor: 'pointer', fontSize: 14, color: C.navy, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              &#8249;
            </button>
            <button onClick={(e) => { e.stopPropagation(); setIdx(i => i < images.length - 1 ? i + 1 : 0); }}
              style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.85)', border: 'none', cursor: 'pointer', fontSize: 14, color: C.navy, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              &#8250;
            </button>
          </>
        )}
        ...
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: 8 }}>
          {images.map((img, i) => (
            <button key={i} onClick={(e) => { e.stopPropagation(); setIdx(i); }}
              style={{
                width: isModal ? 48 : 32, height: isModal ? 48 : 32, borderRadius: 4, overflow: 'hidden',
                border: i === idx ? `2px solid ${C.navy}` : `1px solid ${C.border}`, cursor: 'pointer', padding: 0,
              }}>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: '95%', maxWidth: 820, maxHeight: '90vh', background: C.white,
        borderRadius: 16, zIndex: 301, overflow: 'auto',
      }}>
        <button onClick={onClose} style={{
          position: 'absolute', top: 16, right: 16, width: 36, height: 36, borderRadius: '50%',
          background: C.gray, border: 'none', cursor: 'pointer', fontSize: 18, color: C.muted, zIndex: 302,
        ...
        <div style={{
          ...
          <div>
            {product.collection && (
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: C.yellow, margin: '0 0 8px' }}>
                {product.collection}
              </p>
            )}
            <h2 style={{ fontFamily: "'Source Serif 4', serif", fontSize: 28, fontWeight: 700, color: C.navy, margin: '0 0 4px' }}>
              {product.name}
            </h2>
            <p style={{ fontSize: 14, color: C.muted, margin: '0 0 8px' }}>{product.description}</p>
            {product.blurb && (
              <p style={{ fontSize: 15, color: C.charcoal, margin: '0 0 16px', lineHeight: 1.5 }}>{product.blurb}</p>
            )}

            <div style={{ fontSize: 28, fontWeight: 800, color: C.navy, margin: '0 0 20px' }}>
            ...
            <button onClick={handleAdd} style={{
              ...
              padding: 16, borderRadius: 8, border: 'none', fontSize: 16, fontWeight: 700,
...
function ProductCard({ product, onOpen }: { product: SwagProduct; onOpen: () => void }) {
  ...
  const handleQuickAdd = (e: React.MouseEvent) => {
    e.stopPropagation();
    const defaultVariant = product.variants?.[2]?.value || product.variants?.[0]?.value;
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={handleQuickAdd} style={{
            background: added ? C.navy : C.yellow, border: 'none',
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch('/api/swag/checkout', {
        ...
        body: JSON.stringify({ items: items.map(i => ({ productId: i.product.id, name: i.product.name, price: i.product.price, quantity: i.quantity, variant: i.variant, image: i.product.images[0] })) }),
      });
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, width: '100%', maxWidth: 400, height: '100vh', background: C.white, boxShadow: '-4px 0 24px rgba(0,0,0,0.1)', zIndex: 200, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 24px' }}>
          {items.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 16px', color: C.muted }}><p style={{ fontSize: 14 }}>Your cart is empty.</p></div>
          ) : items.map(item => (
        ...
        {items.length > 0 && (
          <div style={{ padding: '20px 24px', borderTop: `1px solid ${C.border}` }}>
            ...
            <p style={{ textAlign: 'center', fontSize: 11, color: C.muted, margin: '10px 0 0' }}>Secure checkout. Ships in 3-5 business days.</p>
          </div>
        )}
      </div>
    </>
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: C.white, minHeight: '100vh' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '64px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 48, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              <a href="#collections" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: C.yellow, color: C.navy, padding: '14px 28px', borderRadius: 6, fontSize: 14, fontWeight: 700, textDecoration: 'none' }}>
                Shop the Drop
      ...
      <div id="collections">
        {COLLECTIONS.map(col => {
          ...
          return (
            <div key={col.key} style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px' }}>
              <div style={{ paddingTop: 48, paddingBottom: 8, borderBottom: `2px solid ${C.navy}`, marginBottom: 24 }}>
                <h2 style={{ fontFamily: "'Source Serif 4', serif", fontSize: 26, fontWeight: 700, color: C.navy, margin: 0 }}>
                  {col.key}
                </h2>
                <p style={{ fontSize: 14, color: C.muted, margin: '4px 0 0' }}>{col.description}</p>
              </div>
              <div style={{
                display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${products.length <= 2 ? '280px' : '240px'}, 1fr))`,
                gap: 28, marginBottom: 16,
              }}>
                {products.map(p => (
      ...
      <div style={{ background: C.gray }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
          <h2 style={{ fontFamily: "'Source Serif 4', serif", fontSize: 26, fontWeight: 700, color: C.navy, margin: '0 0 16px' }}>
            The Sticker Bar
          </h2>
      ...
      <div style={{ background: C.navy, padding: '48px 24px', textAlign: 'center' }}>
        <p style={{ fontFamily: "'Source Serif 4', serif", fontSize: 22, fontWeight: 700, color: C.white, margin: '0 0 8px' }}>
          Outfitting a team?
        </p>
        ...
        <a href="/contact" style={{
          display: 'inline-flex', alignItems: 'center', gap: 8, background: C.yellow, color: C.navy,
          padding: '14px 28px', borderRadius: 6, fontSize: 14, fontWeight: 700, textDecoration: 'none',
        }}>
