...
import { postFundingDigest } from '@/lib/funding-slack'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (alerts.length > 0) {
      ...
      // Posted to the grant channel rather than emailed.
      //
      // This used to email rae@teachersdeserveit.com and nobody else. Bella
      // does the work and lives in Slack, so the person who needed the digest
      // never saw it and the person who saw it was not the one acting on it.
      // Slack was already enabled with a working webhook and both handles
      // stored, and the digest simply did not use it.
      if (!dryRun) {
        const posted = await postFundingDigest(
          critical.map(a => ({ pursuitName: a.pursuit_name, message: `${a.title}. ${a.action}` })),
          warnings.map(a => ({ pursuitName: a.pursuit_name, message: a.title })),
        )
        if (!posted) {
          console.error('[funding-reminders] Funding digest was not delivered. Slack is disabled, unconfigured, or the post failed.')
        }
      }
    }
...
// buildDigestEmail removed. The digest posts to the grant channel in Slack
// now, so the HTML email builder had no caller. Left as a note rather than a
// silent deletion, because the next person to want an emailed digest should
// know one existed and why it went.
