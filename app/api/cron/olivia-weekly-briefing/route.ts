...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!resendApiKey) {
      console.error('[olivia-weekly-briefing] RESEND_API_KEY not set');
    ...
    if (!emailRes.ok) {
      console.error('[olivia-weekly-briefing] Resend failed:', emailRes.status, emailResult);
      return NextResponse.json({ error: 'Email delivery failed', status: emailRes.status, details: emailResult }, { status: 502 });
    }

    console.log('[olivia-weekly-briefing] Sent:', subject, 'emailId:', emailResult.id);
