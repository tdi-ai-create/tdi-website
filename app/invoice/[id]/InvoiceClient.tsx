...
export default function InvoiceClient({ quote: initialQuote }: { quote: Quote }) {
  ...
  const [signed, setSigned] = useState(initialQuote.status === 'signed')
  ...
  // Track view on mount
  useEffect(() => {
    if (!viewTracked.current && ['sent', 'viewed'].includes(quote.status)) {
      viewTracked.current = true
      fetch(`/api/quotes/${quote.id}/view`, { method: 'POST' })
    }
  ...
  async function handleSign() {
    ...
    if (data.success) {
      setSigned(true)
      setQuote(prev => ({ ...prev, status: 'signed', signed_by_name: signedName, signed_at: new Date().toISOString() }))
    }
