              ...
              <div className="bg-white border-2 rounded-xl p-5 md:p-6 shadow-md" style={{ borderColor: '#ffba06' }}>
                ...
                <div className="mb-3 pt-3 border-t" style={{ borderColor: '#e5e7eb' }}>
                  ...
                  <ul className="space-y-1">
                    {['2 On-Campus Observation Days', '4 Virtual Strategy Sessions', '2 Executive Impact Sessions', 'Learning Hub access for pilot group', 'Leadership Dashboard'].map((item, i) => (
                      <li key={i} className="flex items-center gap-1.5 text-xs" style={{ color: '#1e2749', opacity: 0.7 }}>
            ...
            <div className="flex-1 pb-8">
              <div className="bg-white border-2 rounded-xl p-5 md:p-6 shadow-md" style={{ borderColor: '#80a4ed' }}>
                ...
                <div className="mb-3 pt-3 border-t" style={{ borderColor: '#e5e7eb' }}>
                  ...
                  <ul className="space-y-1">
                    {['Learning Hub access for ALL staff', '4 Executive Impact Sessions', 'Teachers Deserve It book for every educator', 'Retention tracking tools'].map((item, i) => (
                      <li key={i} className="flex items-center gap-1.5 text-xs" style={{ color: '#1e2749', opacity: 0.7 }}>
            ...
            <div className="flex-1">
              <div className="bg-white border-2 rounded-xl p-5 md:p-6 shadow-md" style={{ borderColor: '#abc4ab' }}>
                ...
                <div className="mb-3 pt-3 border-t" style={{ borderColor: '#e5e7eb' }}>
                  ...
                  <ul className="space-y-1">
                    {['Desi AI Assistant (24/7 support)', 'Advanced analytics', 'Ongoing partnership support'].map((item, i) => (
                      <li key={i} className="flex items-center gap-1.5 text-xs" style={{ color: '#1e2749', opacity: 0.7 }}>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mb-4" style={{ color: '#1e2749', opacity: 0.8 }}>
          Every teacher we observe receives a Love Note, a personalized note highlighting specific strengths we saw in their classroom.
        </p>
        <div
        ...
        >
          ...
          <p className="text-sm italic leading-relaxed" style={{ color: '#1e2749' }}>
            "During your small group rotation today, I noticed how you used proximity and a calm voice to redirect Marcus without stopping instruction. The other students did not even look up. That is classroom management mastery."
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-8">
      ...
      <div className="p-4 rounded-lg" style={{ backgroundColor: '#f0f9ff', border: '1px solid #80a4ed' }}>
        ...
        <p className="text-sm" style={{ color: '#1e2749', opacity: 0.8 }}>
          Most PD has a 10% implementation rate. Ours is 65%. The difference is in the design.
        </p>
      </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="rounded-xl overflow-hidden shadow-lg" style={{ border: '1px solid #e5e7eb' }}>
          ...
          <div className="p-4 grid grid-cols-2 gap-3" style={{ backgroundColor: '#f5f5f5' }}>
            {[
              { title: 'Staff Engagement', desc: 'Who is logging in, completing courses' },
              { title: 'Implementation Progress', desc: 'Which strategies are being used' },
              { title: 'Observation Insights', desc: 'Trends from on-campus visits' },
              { title: 'Wellness Trends', desc: 'Staff stress and satisfaction over time' },
            ].map((item, i) => (
              <div key={i} className="p-3 rounded-lg bg-white">
                <p className="text-xs font-medium mb-1" style={{ color: '#1e2749', opacity: 0.6 }}>{item.title}</p>
                <p className="text-sm" style={{ color: '#1e2749' }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="p-4 rounded-lg" style={{ backgroundColor: '#fffbeb', border: '1px solid #ffba06' }}>
        ...
        <p className="text-sm" style={{ color: '#1e2749', opacity: 0.8 }}>
          When renewal conversations come up, you will have the data. Not just "teachers liked it" but "here is the measurable change we saw."
        </p>
      </div>
    </div>
  );
        ...
        <div className="flex-1 space-y-4">
          <div>
            ...
            <p style={{ color: '#1e2749', opacity: 0.8 }}>
              Teachers Deserve It is a practical guide for educators who want to reclaim their time, rebuild their confidence, and remember why they started teaching in the first place.
            </p>
          </div>
          <div className="p-4 rounded-lg" style={{ backgroundColor: '#f5f5f5' }}>
            ...
            <p className="text-sm italic" style={{ color: '#1e2749', opacity: 0.8 }}>
              "This is not a book about doing more. It is about doing what matters."
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #e5e7eb' }}>
          ...
          <div className="grid grid-cols-3 text-sm border-b" style={{ borderColor: '#e5e7eb' }}>
            <div className="p-3 border-r" style={{ borderColor: '#e5e7eb', color: '#ef4444' }}>10% industry avg</div>
            <div className="p-3 border-r" style={{ borderColor: '#e5e7eb', color: '#22c55e' }}>65% with TDI</div>
            <div className="p-3" style={{ color: '#1e2749' }}>Implementation rate</div>
          </div>
        </div>
      ...
      <div className="p-4 rounded-lg" style={{ backgroundColor: '#f0f9ff', border: '1px solid #80a4ed' }}>
        ...
        <p className="text-sm" style={{ color: '#1e2749', opacity: 0.8 }}>
          The goal is not to complete a course. The goal is for a teacher to try a new strategy on Monday and see it work by Friday.
        </p>
      </div>
    </div>
  );
...
export function HowWePartnerTabs({ excludeTabs = [], showCTAs = true }: HowWePartnerTabsProps) {
  const tabs = allTabs.filter(tab => !excludeTabs.includes(tab.id));
  const [activeTab, setActiveTab] = useState<TabId>(tabs[0]?.id || 'approach');
  const [mobileExpanded, setMobileExpanded] = useState<TabId | null>(tabs[0]?.id || 'approach');
  ...
  const renderPanel = (tabId: TabId) => {
    const content = (() => {
      switch (tabId) {
        case 'approach': return <OurApproachPanel />;
        case 'in-person': return <InPersonPanel />;
        case 'learning-hub': return <LearningHubPanel />;
  ...
  return (
    <div>
      ...
      <div className="hidden lg:flex gap-8">
        ...
        <div ref={contentRef} className="flex-1 bg-white rounded-2xl p-6 shadow-lg" style={{ border: '1px solid #e5e7eb' }}>
